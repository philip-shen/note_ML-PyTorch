'''
pytorch_advanced/2_objectdetection/2-4-5_SSD_model_forward.ipynb

https://github.com/YutaroOgawa/pytorch_advanced/blob/master/2_objectdetection/2-4-5_SSD_model_forward.ipynb
'''
...
def chk_make_vgg():
    ...
    vgg_test = make_vgg()
    #print(vgg_test)
    logger.info(f'vgg_test: {vgg_test}')
...
def chk_make_extras():
    ...
    #print(extras_test)
    logger.info(f'extras_test: {extras_test}')

# デフォルトボックス(defalut box)のオフセット(offset)を出力するloc_layers、
...
'''
def chk_make_loc_conf():
    # 動作確認
    loc_test, conf_test = make_loc_conf()
    logger.info(f'loc_test: {loc_test}')
    logger.info(f'conf_test: {conf_test}')
...
def chk_DBox():
    ...
    # DBoxの出力を確認する
    dbox_list_pd = pd.DataFrame(dbox_list.numpy())

    logger.info(f'dbox_list_pd: {dbox_list_pd}')
...
def chk_SSD():
    ...
    logger.info(f'ssd_test: {ssd_test}')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='ML_Pytorch_22-2_SSD_model_forward')
