...
from libs.data_augumentation import Compose, ConvertFromInts, ToAbsoluteCoords, PhotometricDistort, Expand, RandomSampleCrop, RandomMirror, ToPercentCoords, Resize, SubtractMeans
...
class Detect(Function):
    ...
    def __init__(self, conf_thresh=0.01, top_k=200, nms_thresh=0.45):
        ...
        logger.info(f'self.nms_thresh: {self.nms_thresh}')
    '''
    https://github.com/YutaroOgawa/pytorch_advanced/issues/71#issuecomment-622761399

    ●変更②前
    class Detect(Function):
    def init(self, conf_thresh=0.01, top_k=200, nms_thresh=0.45):
...
class SSD(nn.Module):
    ...
    def __init__(self, phase, cfg):
        ...
        if phase == 'inference':
            self.detect = Detect()
            logger.info(f'self.detect: {self.detect}')
    ...
    def forward(self, x):
        ...
        if self.phase == "inference":  # 推論時
            ...
            logger.info(f'self.detect(output[0], output[1], output[2]): {self.detect(output[0], output[1], output[2])}')
