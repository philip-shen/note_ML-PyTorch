'''
    ssd-pytorch/utils/utils_fit.py

https://github.com/bubbliiiing/ssd-pytorch/blob/master/utils/utils_fit.py
'''
import os
...
def fit_one_epoch(model_train, model, ssd_loss, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, fp16, scaler, save_period, save_dir, local_rank=0):
    ...
    if local_rank == 0:
        ...
        logger.info('Total Loss: {:.3f} || Val Loss: {:.3f} '.format(total_loss / epoch_step, val_loss / epoch_step_val))
...
'''
    pspnet-pytorch/utils/utils_fit.py

https://github.com/bubbliiiing/pspnet-pytorch/blob/master/utils/utils_fit.py
'''        

from nets.pspnet_training import CE_Loss, Dice_loss, Focal_Loss
from .utils_metrics import f_score

def fit_one_epoch_pspnet(model_train, model, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val, gen, gen_val, Epoch, cuda, dice_loss, focal_loss, cls_weights, aux_branch, num_classes, \
