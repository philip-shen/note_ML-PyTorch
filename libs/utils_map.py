'''
    ssd-pytorch/utils/utils_map.py

https://github.com/bubbliiiing/ssd-pytorch/blob/master/utils/utils_map.py
'''
import glob
import json
import math
...
"""
 throw error and exit    ssd-pytorch/utils
    ...
    with open(RESULTS_FILES_PATH + "/results.txt", 'w') as results_file:
        ...
        for class_index, class_name in enumerate(gt_classes):
            ...
            if len(prec)>0:
                ...
                msg = text + "\t||\tscore_threhold=" + str(score_threhold) + " : " + "F1=" + "{0:.2f}".format(F1[score_threhold_idx])\
                    + " ; Recall=" + "{0:.2f}%".format(rec[score_threhold_idx]*100) + " ; Precision=" + "{0:.2f}%".format(prec[score_threhold_idx]*100)
                logger.info(msg)
            else:
                #print(text + "\t||\tscore_threhold=" + str(score_threhold) + " : " + "F1=0.00% ; Recall=0.00% ; Precision=0.00%")
                msg = text + "\t||\tscore_threhold=" + str(score_threhold) + " : " + "F1=0.00% ; Recall=0.00% ; Precision=0.00%"
                logger.info(msg)
        ...
        if n_classes == 0:
            ...
            logger.info("未检测到任何种类，请检查标签信息与get_map.py中的classes_path是否修改。")
            return 0
        results_file.write("\n# mAP of all classes\n")
