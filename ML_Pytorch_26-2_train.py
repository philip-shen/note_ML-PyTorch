'''
    pspnet-pytorch/train.py

https://github.com/bubbliiiing/pspnet-pytorch/blob/master/train.py
'''
import os, sys, time
import datetime
...
from nets.pspnet_training import (get_lr_scheduler, set_optimizer_lr,
                                  weights_init)
from libs.callbacks import EvalCallback, LossHistory
from libs.dataloader import PSPnetDataset, pspnet_dataset_collate
from libs.utils import (download_weights, seed_everything,
                         show_config, worker_init_fn)
from libs.utils_fit import fit_one_epoch_pspnet
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='ML_Pytorch_26-2_train')
    ...
    #                   自己需要的分类个数+1，如2+1
    #-----------------------------------------------------#
    num_classes     = json_data["Config"][9]["num_classes"]
    #-------------------------------#
    #   主干网络选择
    #   mobilenet、resnet50
    #-------------------------------#
    backbone        = json_data["Config"][9]["backbone"]
    #----------------------------------------------------------------------------------------------------------------------------#
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    model_path      = json_data["Config"][9]["model_path"]
    #---------------------------------------------------------#
    #   downsample_factor   下采样的倍数8、16 
    #                       8下采样的倍数较小、理论上效果更好。
    #                       但也要求更大的显存
    #---------------------------------------------------------#
    downsample_factor   = json_data["Config"][9]["downsample_factor"]
    #-------------------------------#
    #   输入图片的大小
    #-------------------------------#
    input_shape         = json_data["Config"][9]["input_shape"]
    ...
    #                       (当Freeze_Train=False时失效)
    #------------------------------------------------------------------#
    Init_Epoch          = json_data["Config"][9]["init_epoch"]
    Freeze_Epoch        = json_data["Config"][9]["freeze_epoch"]
    Freeze_batch_size   = json_data["Config"][9]["freeze_batch_size"]
    #------------------------------------------------------------------#
    #   解冻阶段训练参数
    ...
    #------------------------------------------------------------------#
    UnFreeze_Epoch      = json_data["Config"][9]["unfreeze_epoch"]
    Unfreeze_batch_size = json_data["Config"][9]["unfreeze_batch_size"]
    #------------------------------------------------------------------#
    ...
    #------------------------------------------------------------------#
    Init_lr             = json_data["Config"][9]["init_lr"]
    Min_lr              = Init_lr * 0.01
    #------------------------------------------------------------------#
    ...
    #------------------------------------------------------------------#
    optimizer_type      = json_data["Config"][9]["optimizer_type"]
    momentum            = json_data["Config"][9]["momentum"]
    weight_decay        = json_data["Config"][9]["weight_decay"]
    #------------------------------------------------------------------#
    #   lr_decay_type   使用到的学习率下降方式，可选的有'step'、'cos'
    #------------------------------------------------------------------#
    lr_decay_type       = json_data["Config"][9]["lr_decay_type"]
    #------------------------------------------------------------------#
    #   save_period     多少个epoch保存一次权值，默认每个世代都保存
    #------------------------------------------------------------------#
    save_period         = json_data["Config"][9]["save_period"]
    #------------------------------------------------------------------#
    ...
    #------------------------------------------------------------------#
    eval_flag           = True
    eval_period         = json_data["Config"][9]["eval_period"]
    ...
    if sys.platform.lower() == 'linux':
        ...
        VOCdevkit_webdav_2007_path  = f'{home}/infinicloud/2007_trainval/VOCdevkit'
    elif sys.platform.lower() == 'win32':
        home = 'z:'
        VOCdevkit_webdav_2007_path  = f'{home}/2007_trainval/VOCdevkit'
    #------------------------------------------------------------------#
    #   建议选项：
    #   种类少（几类）时，设置为True
    ...
    #------------------------------------------------------------------#
    num_workers         = json_data["Config"][9]["num_workers"]
    ...
    if distributed:
        ...
        if local_rank == 0:
            ...
            logger.info(f"Gpu Device Count : { ngpus_per_node}")
    else:
    ...
    #   读取数据集对应的txt
    #---------------------------#
    with open(os.path.join(VOCdevkit_webdav_2007_path, "VOC2007/ImageSets/Segmentation/train.txt"),"r") as f:
        train_lines = f.readlines()
    with open(os.path.join(VOCdevkit_webdav_2007_path, "VOC2007/ImageSets/Segmentation/val.txt"),"r") as f:
    ...
    if local_rank == 0:
        ...
        if total_step <= wanted_step:
            ...
            logger.info(f"\033[1;33;44m[Warning] 本次运行的总训练数据量为{num_train}，Unfreeze_batch_size为{Unfreeze_batch_size}，共训练{UnFreeze_Epoch}个Epoch，计算出总训练步长为{total_step}。\033[0m")
            logger.info(f"\033[1;33;44m[Warning] 由于总训练步长为{total_step}，小于建议总步长{wanted_step}，建议设置总世代为{wanted_epoch}。\033[0m")
    ...
    if True:
        ...
        gen             = DataLoader(train_dataset, shuffle = shuffle, batch_size = batch_size, \
                                    ...
                                    collate_fn = pspnet_dataset_collate, sampler=train_sampler, 
                                    worker_init_fn=partial(worker_init_fn, rank=rank, seed=seed))
        
        gen_val         = DataLoader(val_dataset  , shuffle = shuffle, batch_size = batch_size, \
                                     num_workers = num_workers, pin_memory=True, drop_last = True, \
                                        collate_fn = pspnet_dataset_collate, sampler=val_sampler, 
                                    worker_init_fn=partial(worker_init_fn, rank=rank, seed=seed))
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            fit_one_epoch_pspnet(model_train, model, loss_history, eval_callback, optimizer, epoch, \
                                 epoch_step, epoch_step_val, gen, gen_val, UnFreeze_Epoch, Cuda, dice_loss, \
                                 focal_loss, cls_weights, aux_branch, num_classes, fp16, scaler, save_period, save_dir, local_rank)
