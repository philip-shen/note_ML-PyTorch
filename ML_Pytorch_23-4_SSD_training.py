...
https://github.com/bubbliiiing/ssd-pytorch/blob/master/train.py
'''
import datetime
import warnings
...
from nets.ssd_training import (MultiboxLoss, get_lr_scheduler,set_optimizer_lr, weights_init)

from libs.anchors import get_anchors
from libs.callbacks import EvalCallback, LossHistory
from libs.dataloader import SSDDataset, ssd_dataset_collate
from libs.utils import (download_weights, voc_labels, seed_everything,
                         show_config, worker_init_fn)

from libs.utils_fit import fit_one_epoch
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='ML_Pytorch_23-4_SSD_training')
    ...
    #           使得每次独立训练都可以获得一样的结果
    #----------------------------------------------#
    seed            = json_data["Config"][5]["seed"]#11
    #---------------------------------------------------------------------#
    ...
    model_path      = f'{home}/infinicloud/{json_data["Config"][5]["model_path"]}'
    #------------------------------------------------------#
    ...
    #------------------------------------------------------#
    backbone        = json_data["Config"][5]["backbone"]#"vgg"
    #----------------------------------------------------------------------------------------------------------------------------#
    ...
    #                       (当Freeze_Train=False时失效)
    #------------------------------------------------------------------#
    Init_Epoch          = json_data["Config"][5]["init_epoch"]##0
    Freeze_Epoch        = json_data["Config"][5]["freeze_epoch"]#50
    Freeze_batch_size   = json_data["Config"][5]["freeze_batch_size"]#16
    #------------------------------------------------------------------#
    #   解冻阶段训练参数
    ...
    #------------------------------------------------------------------#
    UnFreeze_Epoch      = json_data["Config"][5]["unfreeze_epoch"]#200
    Unfreeze_batch_size = json_data["Config"][5]["unfreeze_batch_size"]#8
    #------------------------------------------------------------------#
    ...
    #------------------------------------------------------------------#
    Init_lr             = json_data["Config"][5]["init_lr"]#2e-3
    Min_lr              = Init_lr * 0.01
    #------------------------------------------------------------------#
    ...
    #------------------------------------------------------------------#
    optimizer_type      = json_data["Config"][5]["optimizer_type"]#"sgd"
    momentum            = json_data["Config"][5]["momentum"]#0.937
    weight_decay        = json_data["Config"][5]["weight_decay"]#5e-4
    #------------------------------------------------------------------#
    #   lr_decay_type   使用到的学习率下降方式，可选的有'step'、'cos'
    #------------------------------------------------------------------#
    lr_decay_type       = json_data["Config"][5]["lr_decay_type"]#'cos'
    #------------------------------------------------------------------#
    #   save_period     多少个epoch保存一次权值
    #------------------------------------------------------------------#
    save_period         = json_data["Config"][5]["save_period"]#10
    #------------------------------------------------------------------#
    #   save_dir        权值与日志文件保存的文件夹
    #------------------------------------------------------------------#
    save_dir            = dirnamelog#'logs'
    #------------------------------------------------------------------#
    ...
    #------------------------------------------------------------------#
    eval_flag           = True
    eval_period         = json_data["Config"][5]["eval_period"]#10
    #------------------------------------------------------------------#
    ...
    #------------------------------------------------------------------#
    num_workers         = json_data["Config"][5]["num_workers"]#4

    #------------------------------------------------------#
    #   train_annotation_path   训练图片路径和标签
    #   val_annotation_path     验证图片路径和标签
    #------------------------------------------------------#
    train_annotation_path   = json_data["Config"][5]["train_annotation_path"]#'2007_train.txt'
    val_annotation_path     = json_data["Config"][5]["val_annotation_path"]#'2007_val.txt'
    ...
    class_names, num_classes  = [k for _, k in enumerate(voc_labels)], voc_labels.__len__()
    ...
    if model_path != '':
        ...
        if local_rank == 0:
            ...
            logger.info("\033[1;33;44m温馨提示，head部分没有载入是正常现象，Backbone部分没有载入是错误的。\033[0m")
